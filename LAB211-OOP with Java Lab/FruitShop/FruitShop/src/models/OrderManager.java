...
public class OrderManager {
    ...
    public void updateQuantity() {
        listOfFruit = p.getList();
        for (Fruit product : listOfFruit) {
            for (Fruit storage : listOfFruit) {
                if (product.getFruitID() == storage.getFruitID()) {
                    int updateQuantity = storage.getQuantity() - product.getQuantity();
                    if (updateQuantity >= 0) {
                        storage.setQuantity(updateQuantity);
                    } else {
                        System.out.println("Error: Insufficient quantity of " + storage.getFruitName() +" in storage.");
                    }
                    break;
                }
            }
